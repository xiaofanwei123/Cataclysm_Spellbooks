...
import net.acetheeldritchking.cataclysm_spellbooks.capabilities.PlayerWrath;
import net.acetheeldritchking.cataclysm_spellbooks.capabilities.PlayerWrathProvider;
...
public class ServerEvents {
    ...
    public void onLivingDeathEvent(LivingDeathEvent event)
    {
        ...
        if (entity instanceof LivingEntity livingEntity)
        {
            ...
            {
                if (!damageSource.is(DamageTypeTags.BYPASSES_INVULNERABILITY) && CSUtils.tryCurisumChestplateRebirth(livingEntity))
                {
    ...
    public void onLivingAttackEvent(LivingAttackEvent event)
    {
        ...
        if (entity instanceof LivingEntity livingEntity)
        {
            ...
            {
                if (event.getSource().is(DamageTypeTags.BYPASSES_INVULNERABILITY) && CSUtils.tryCurisumChestplateRebirth(livingEntity))
                {
                    if (livingEntity.getRandom().nextFloat() < 0.15F)
                    {
                        event.setCanceled(true);
                    }
                } else if (!event.getSource().is(DamageTypeTags.BYPASSES_INVULNERABILITY) && CSUtils.tryCurisumChestplateRebirth(livingEntity))
                {
    ...
    public static void onAttachCapabilitiesPlayer(AttachCapabilitiesEvent<Entity> event)
    {
        if (event.getObject() instanceof Player)
        {
            ...
            {
                event.addCapability(new ResourceLocation(Cataclysm_Spellbooks.MODID, "wrath"), new PlayerWrathProvider());
            }
        }
    }
